...
`specdrift update` はハッシュ変更の詳細を出力します：

```
docs/spec/auth.md: 2 updated (reason: ハンドラをリファクタリング、仕様は正確)
  src/auth/handler.go: a1b2c3d4 -> e5f6a7b8
  src/auth/token.go: 11223344 -> 55667788
```

すべての update に理由を必須にするには、`.specdrift` に設定します：

```json
{"require_reason": true}
```

この設定により、`--reason` なしの `specdrift update` はエラーになります。理由は出力に含まれ、会話ログや CI の出力から追跡可能です。

```bash
specdrift update --reason "仕様レビュー済み、新APIに対応" 'docs/spec/*.md'
```
...
- 更新（レビュー後）: `specdrift update --reason "<理由>" 'docs/spec/*.md'`
