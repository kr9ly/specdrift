...
5. **coverage check**: `go run . coverage --src 'internal/*.go' --src '*.go' 'docs/spec/*.md'`
   ...
   - If the user wants to add documentation, update the relevant spec files, run `go run . update --reason "<reason>" 'docs/spec/*.md'` to sync hashes, then re-run steps 3-4 before proceeding.
